...
shot_locs = SkyCoord(ra=np.array(T['ra']) * u.deg, dec=np.array(T['dec']) * u.deg, frame='icrs')
datevobs, shot_names = T['shotid'], T['name']
...
def _find_shots(target_coord):
    angular_seps = target_coord.separation(shot_locs)
    sel = (angular_seps < 0.15*u.deg)
